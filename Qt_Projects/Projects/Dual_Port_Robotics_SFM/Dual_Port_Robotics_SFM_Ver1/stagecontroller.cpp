...
void stageController::startScan()
{
        ...
        {
            connect(scannerThread, SIGNAL(started()), scannerObj, SLOT(RectangularScan()));

        }
        else if (settingInfoPtr->Materialshape == "Cylindrical")
        {
            connect(scannerThread, SIGNAL(started()), scannerObj, SLOT(CylindricalScan()));
        }
        else if (settingInfoPtr->Materialshape == "Leading Edge Specimen")
        {
            connect(scannerThread, SIGNAL(started()), scannerObj, SLOT(CoordinateScan()));
        }
...
void scanner::Zeroscan()
{
           ...
           vStageSuccess && hStageSuccess)
    {
        ...
        else
        {
            ...
            {
                Xpoint++;
                if(Xpoint==settingInfoPt->xLength)//If last Point
                {
                    rotstageTargetPos += 0;//settingInfoPt->DegreeIntervalV[Zpoint][0];
                    rotStageSuccess = rotStageProc->setPosAbs(rotstageTargetPos);
                    hstageTargetPos += 0;//settingInfoPt->LinearIntervalV[Zpoint][0];
                    hStageSuccess = hstageProc->setPosAbs(hstageTargetPos);
                }
                else
                {
