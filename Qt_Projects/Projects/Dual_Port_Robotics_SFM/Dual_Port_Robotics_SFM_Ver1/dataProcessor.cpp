...
void dataProcessor::loadCoordinates(bool status)
{
    ...
    buffScanWidthLoc    = settingInfoPtr->scanWidth/10+1;// settingInfoPtr->xLength;
...
{
    int buffScanIntervalLoc = settingInfoPtr->scanInterval;
    int buffScanX10WidthLoc = settingInfoPtr->scanWidth/10+1;
    int buffScanX5WidthLoc  = settingInfoPtr->scanWidth/5+1;
    int buffScanWidthLoc    = settingInfoPtr->xLength;
    int buffScanHeightLoc   = settingInfoPtr->ZPoints;
    //Variables for rearranging
    ...
    for (int j=0;j<buffScanHeightLoc;j++)
    {
        ...
        //Calculate Extra Points and arrange coordinates
        if(runtime == 0) //if first time than calculate for 5 mm
        {
            ...
            for (int i=0; i<buffScanX10WidthLoc;i++)
            {
            ...
            special_Xn[j] = 3*Coordinates[j][0][buffScanX10WidthLoc-1]-3*Coordinates[j][0][buffScanX10WidthLoc-2]+ Coordinates[j][0][buffScanX10WidthLoc-3];
            special_Yn[j] = 3*Coordinates[j][1][buffScanX10WidthLoc-1]-3*Coordinates[j][1][buffScanX10WidthLoc-2]+ Coordinates[j][1][buffScanX10WidthLoc-3];

            for (int i=0; i<(buffScanX10WidthLoc+1);i++)
            {
                ...
                else if  (0 < i &&  i < buffScanX10WidthLoc)
                {
                ...
                else if (i == buffScanX10WidthLoc)        // if last point
                {
            ...
            for (int i=0;i<buffScanX10WidthLoc;i++)
            {
                ...
                if (Coordinates[0][1][i]>Center_of_Rotation[1]) // if Y value of coordinate if greater than center Y value
                {
                    ...
                    Center_of_Rotation[0] = Coordinates[0][0][i];
                    CentralValue_X = i;
                }
            }
            ...
            if(settingInfoPtr->AchieveLowIA)//Reduce incident angle by utilizing both the stages
            {
                ...
                for(int i=0;i<buffScanX10WidthLoc;i++)
                {
                    if (i==0)
                    {
                        settingInfoPtr->DegreeIntervalV[j][i] = Incident_Angle_L[j][CentralValue_Y][i];
            ...
            for (int i=0; i<(buffScanX10WidthLoc+1);i++)
            {
                ...
                if(i==buffScanX10WidthLoc)
                {
                    Extra_XCoor[j][i] = (special_Xn[j]+Coordinates[j][0][i])/2;
                    Extra_YCoor[j][i] = (special_Yn[j]+Coordinates[j][1][i])/2;
                    Arranged_XCoor[j][i*2]   = Coordinates[j][0][i];
                    Arranged_YCoor[j][i*2]   = Coordinates[j][1][i];
                    Arranged_XCoor[j][i*2+1] = Extra_XCoor[j][i];
                    Arranged_YCoor[j][i*2+1] = Extra_YCoor[j][i];
                }
                else
                {
                    Extra_XCoor[j][i] = (Coordinates[j][0][i]+Coordinates[j][0][i+1])/2;
                    Extra_YCoor[j][i] = (Coordinates[j][1][i]+Coordinates[j][1][i+1])/2;
                    Arranged_XCoor[j][i*2]   = Coordinates[j][0][i];
                    Arranged_YCoor[j][i*2]   = Coordinates[j][1][i];
                    Arranged_XCoor[j][i*2+1] = Extra_XCoor[j][i];
                    Arranged_YCoor[j][i*2+1] = Extra_YCoor[j][i];
        ...
        for (int i=0; i<(buffScanX5WidthLoc);i++)
        {
            Coordinates[j][0][i]   = 0;
            Coordinates[j][1][i]   = 0;
        }

        //pick coordinates based on interval
        int x = buffScanIntervalLoc/5;

        for (int i=0; i<(buffScanWidthLoc);i++)
        {

            if((i*x >= buffScanX5WidthLoc))
            {
                Coordinates[j][0][i]   = Arranged_XCoor[j][buffScanX5WidthLoc-1];
                Coordinates[j][1][i]   = Arranged_YCoor[j][buffScanX5WidthLoc-1];
            }
            else
            {
                Coordinates[j][0][i]   = Arranged_XCoor[j][(i*x)];
                Coordinates[j][1][i]   = Arranged_YCoor[j][(i*x)];
            }
        ...
        settingInfoPtr->CenterOfRotation[0]= Coordinates[0][0][CentralValue_X];
