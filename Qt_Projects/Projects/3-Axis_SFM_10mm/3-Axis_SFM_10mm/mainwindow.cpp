    ...
    ui(new Ui::MainWindow)
{
    ...
    //stageProc->rotStageProc    = new rotStageController((structSetting *)&settingInfo);
    ...
    //Rot Stage Jog connection
    //Salah start
    connect(ui->pushButtonRotOrigin,SIGNAL(clicked(bool)),stageProc->rotStageProc, SLOT(home()));
    connect(ui->pushButtonRotClockWise,SIGNAL(pressed()),stageProc->rotStageProc, SLOT(spinClockWise()));
    connect(ui->pushButtonRotAClockWise,SIGNAL(pressed()),stageProc->rotStageProc, SLOT(spinAntiClockWise()));
    connect(ui->pushButtonRotClockWise,SIGNAL(released()),stageProc->rotStageProc, SLOT(stop()));
    connect(ui->pushButtonRotAClockWise,SIGNAL(released()),stageProc->rotStageProc, SLOT(stop()));

    connect(ui->pushButtonRotGetPos,SIGNAL(pressed()),stageProc->rotStageProc, SLOT(getPos()));
    //Salah ends
    ...
    //connect(stageProc->hstageProc,SIGNAL(curPos(QString)),ui->lcdNumberGetPosDeg, SLOT(display(QString)));
    //Salah
    connect(stageProc->rotStageProc,SIGNAL(pos(QString)),ui->lcdNumberGetPosDeg, SLOT(display(QString)));
//slah end
    stageProc->hstageProc->getPos();
    stageProc->vstageProc->getPos();
   // stageProc->rotStageProc->getPos();
    ...
    //Salah Load Coordinates and Graphs
...
void MainWindow::LoadComboBoxLists()
{
    ...
    ui->enumMaterialShape->addItem("Circular_Horizontal");
    ui->enumMaterialShape->addItem("Circular_Vertical");
    ...
    ui->enumGraphType->addItem("Final_IA_rotational_stage");
    ui->enumGraphType->addItem("Final_IA_Linear_stage");
...
void MainWindow::InitSettingPars()
{
    ...
    ui->enumMaterialShape->setCurrentIndex(3);
    if (ui->enumMaterialShape->currentIndex()== 3)
            {
                ui->labelRadius->setText("Radius");
                ui->labelHeightCoordinates->setText("Height Coordinates");
...
void MainWindow::UpdateSettingParStruct()
{
    ...
    else if (ui->enumMaterialShape->currentIndex()== 1)
    {
        ...
        settingInfo.scanWidth          = ui->lineEditScanHeight->text().toInt();
        settingInfo.scanHeight         = ui->lineEditArclength->text().toInt();
        settingInfo.yLength            = (settingInfo.scanHeight/settingInfo.scanInterval)+1;
        settingInfo.xLength            = (settingInfo.scanWidth/settingInfo.scanInterval)+1;
        settingInfo.ScanDegree         = (settingInfo.ArcLength/settingInfo.Radius)*57.30;
        settingInfo.DegreeInterval     = (settingInfo.ScanDegree/(settingInfo.yLength-1));
        settingInfo.Materialshape      = "Circular_Horizontal";
    ...
    else if (ui->enumMaterialShape->currentIndex()== 2)
    {
        ...
        settingInfo.Materialshape      = "Circular_Vertical";

    }
    else if (ui->enumMaterialShape->currentIndex()== 3)
    {
...
void MainWindow::on_pushButtonRotSet_released()
{
    stageProc->rotStageProc->setPos(ui->lineEditSetPosDeg->text().toFloat());
}
...
void MainWindow::makePlot(int x)
{
    ...
    else if (x == 4)
    {
        ...
        {
          x[i] = i;
          y[i] = dataProc->Final_IA_R[0][dataProc->CentralValue_Y][i];
        }
        x1[0]=0;
        y1[0]=0;
        ui->customPlot->graph(0)->setData(x, y);
    ...
    else if (x == 5)
    {
        ...
        {
          x[i] = i;
          y[i] = dataProc->Final_IA_L[0][dataProc->CentralValue_Y][i];
        }
        x1[0]=0;
        y1[0]=0;
        ui->customPlot->graph(0)->setData(x, y);
