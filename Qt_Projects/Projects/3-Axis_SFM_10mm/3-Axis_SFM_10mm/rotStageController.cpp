...
rotStageController::rotStageController(structSetting * scanInfoPtr_arg)
{
...
float rotStageController::getPos()
{
    ...
    emit pos(strPos);
    position = bytePos.toFloat();
    qDebug()<<"rotStageController::getPos: "<<position;
    return position;//strPos.toFloat(); //-1;//91.125;

    //QString s_data = QString::fromAscii(data.data());Z
}
...
bool rotStageController::setPosAbs(float position)
{
    qDebug()<<"rotStageController::setPosAbs: "<<position;
    char cmdString[100];
    sprintf(cmdString, "GroupMoveAbsolute(SP.pos,%.3f)", position);
