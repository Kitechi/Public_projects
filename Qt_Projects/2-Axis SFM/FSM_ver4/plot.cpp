    ...
    QwtPlot( parent )
{
    ...
    zoomerV->setMousePattern( QwtEventPattern::MouseSelect2,
        Qt::RightButton, Qt::ControlModifier );
    zoomerV->setMousePattern( QwtEventPattern::MouseSelect3,
        Qt::RightButton );
...
void Plot::UpdatePlot()
{
    ...
    case measType::ur:
    case measType::ui:
        setPlotAttributes("","",-100,100,"Frequency(GHz)",settingInfoPtr->PnaStartFreq,
                          settingInfoPtr->PnaStopFreq);
        break;

    case measType::s11Mag:
    case measType::s21Mag:
        setPlotAttributes("","Log Mag",-40,0,"Frequency(GHz)",settingInfoPtr->PnaStartFreq,
                          settingInfoPtr->PnaStopFreq);
        break;
...
void Plot::UpdateCurve(float *y_pt, QString Title,float *x_pt)
{
   ...
   /*
   for (int j =0;j<10;j++)
       qDebug()<<"Plot::UpdateCurve"<<x_pt_d_loc[j]<<y_pt_d_loc[j];
   */
