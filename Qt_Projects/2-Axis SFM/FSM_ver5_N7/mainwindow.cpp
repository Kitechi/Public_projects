    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->pushButtonServoKill,SIGNAL(pressed()),this, SLOT(Stop()));
...
{
    stage->setServoStop();
    stage->setPosX(0,true);
    stage->setPosZ(0,true);
    pnaProc->show();
}
...
void MainWindow::on_pushButtonSetpos_released()
{
    bool vretVal = false;
    bool hretVal = false;
    stage->setPosX(ui->lineEditXPos->text().toDouble()*1000,false);
    stage->setPosZ(ui->lineEditZPos->text().toDouble()*1000,false);
//    vretVal = stage->MoveAbsPosZ(ui->lineEditZPos->text().toDouble()*1000);
//    hretVal = stage->MoveAbsPosX(ui->lineEditXPos->text().toDouble()*1000);
