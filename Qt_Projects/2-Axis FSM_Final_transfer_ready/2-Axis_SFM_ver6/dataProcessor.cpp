...
void dataProcessor::takeScreenshot()
{
    ...
    QString captureFileName = "Screenshot"+now.toString("ddMMyy_hhmmss");
    ...
    if (screenShotResult == NULL)
    {
     ...
     if (f!=NULL)
        title.sprintf("%.3f GHz",f[frameNum]);
     else
         title.sprintf("%.3f GHz",settingInfoPtr->PnaStartFreq+ (double)frameNum*settingInfoPtr->PnaFreqStepSize);

    qwtSpectrogram2->updateAxisXY();
    //qwtSpectrogram2->updateData((dataBundle[dispBuffIndex][frameNum]),captureFileName);
    qwtSpectrogram2->updateData(screenShotResult,title);
...
void dataProcessor::saveDataCur(bool status)
{
   ...
   QDateTime now = QDateTime::currentDateTime();
   QString dataStoragePath = this->progDataPath + "\\Scan_Data\\Scan_Data"+now.toString("ddMMyy_hhmmss")+"\\";
