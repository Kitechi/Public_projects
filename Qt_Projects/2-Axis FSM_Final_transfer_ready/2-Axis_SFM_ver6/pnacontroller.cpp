...
void pnaController::savePointData(void)
{
    ofstream myfile;
    QString filepath;
    QString StatusTip;

    if (pMeas->f[0] != settingInfoPtr->PnaStartFreq)
    {
    ...
    {
        int result;
        QMessageBox msgBox(QMessageBox::Question, tr("Scan Data"),tr("Current 1D data has already been saved. Do you want to save it again ?"),
   ...
   QString dataStoragePath = "C:\\FSM_Data\\Point_Data\\Point_Data"+now.toString("ddMMyy_hhmmss")+"\\";
   if( QDir(dataStoragePath).exists() == false)
       QDir().mkpath(dataStoragePath);

    filepath = dataStoragePath+"data.csv";
    myfile.open(filepath.toStdString().c_str(), ios::out);

    myfile << "freq" << "," << "E_real" << "," << "E_imag" << "," << "U_real" << "," << "U_imag" << "," << "S11" << "," << "S21" << ","<<endl;
    for(int smplNum = 0; smplNum<settingInfoPtr->PnaSweepPoints; smplNum++)
    {
        myfile << pMeas->f[smplNum] << ","
               << pMeas->e.real[smplNum] << ","
               << pMeas->e.imag[smplNum] << ","
               << pMeas->u.real[smplNum] << ","
               << pMeas->u.imag[smplNum] << ","
               << pMeas->s11.mag[smplNum] << ","
               << pMeas->s21.mag[smplNum] << "," << endl;

        emit updateProgressBar_pnaControllerSignal(( (double)(smplNum+1)/settingInfoPtr->PnaSweepPoints)*100);
        QTextStream(&StatusTip)<<"Saving 1D Data "<<smplNum+1<<"/"<<settingInfoPtr->PnaSweepPoints<<"sweep points";
        emit updateStatusBar_pnaControllerSignal(StatusTip);
