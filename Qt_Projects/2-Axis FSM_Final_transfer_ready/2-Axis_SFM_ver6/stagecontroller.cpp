...
{
    if(getPosZ() < 1010*1000 || getPosZ() > 1010*10000)
    Jogzp(true);
}

void stageController::JogznStart()
{
    if(getPosZ()> 0 && getPosZ() < 1010*10000)
        Jogzn(true);
    else
    {
        QMessageBox msgBox(QMessageBox::Critical, tr("Home Error"),tr("Please click the origin button."), 0);
        msgBox.exec();
        return;
    }
}
...
{
    if(getPosX()< 1010*1000 || getPosX() > 1010*10000)
    Jogxp(true);
}

void stageController::JogxnStart()
{
    if(getPosX()> 0 && getPosX() < 1010*10000)
        Jogxn(true);
    else
    {
        QMessageBox msgBox(QMessageBox::Critical, tr("Home Error"),tr("Please click the origin button."), 0);
...
void stageController::MoveToScanPoint(int xPos,int zPos)
{
    unsigned int TargetXpos, TargetZpos;
    if (isFirstTime)
    {
        IntitialXpos = getPosX();
        IntitialZpos = getPosZ();
        isFirstTime = false;
    }
    TargetXpos = IntitialXpos - xPos * settingInfoPtr->scanInterval *1000;
    TargetZpos = IntitialZpos + zPos * settingInfoPtr->scanInterval *1000;
    MoveAbsPosZ(TargetZpos);
    MoveAbsPosX(TargetXpos);

}

bool stageController::initialPos (void)
{
    bool success = false;
    int runtime = 0;
    while ((!(scanStartPosX == getPosX()) || !(scanStartPosZ == getPosZ())) && runtime < 100)
    {
        QCoreApplication::processEvents();
        setErrorReset();
        success = MoveAbsPosX(scanStartPosX);
        success = MoveAbsPosZ(scanStartPosZ);
        runtime++;
    }
    return success;
}
...
void scanner::scan()
{
    int i;
    currScanPt = 0;
    double vstageTargetPos = 0,hstageTargetPos = 0;
    ...
    while (currScanPt < settingInfoPt->ScanPoints && stopScan == false)
    {
        //record signal
        qDebug()<<"Current Point" <<currScanPt;
        stageProc->getPosX();
        stageProc->getPosZ();
        while(settingInfoPt->playScan ==false);
        QCoreApplication::processEvents();
        measDone = false;
        Sleep(500);
        ...
        if (((currScanPt+1)%settingInfoPt->xLength) == 0)
        {
            ...
            stageProc->setPosZ(unsigned int (vstageTargetPos),true);
        }
        ...
        else
        {
            if ((currScanPt/settingInfoPt->xLength)%2 == 0 ) // even row
            {
                ...
                stageProc->setPosX(unsigned int (hstageTargetPos),true);
            }
    ...
    stageProc->initialPos();
    stageProc->isFirstTime = true;
    settingInfoPt->scanstart = false;
    emit finished(i);
}
