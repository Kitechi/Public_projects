...
daqController::~daqController()
{
    ...
    if(this->scannerObj!=NULL) // incase previous thread is running, exit it before closing
    {
        StopAcquisition();
        //while(this->scannerObj!=NULL);//wait to for the exit of prev thread
...
void daqController::Configure( bool OsciModeEnArg)
{
    ...
    if(this->scannerObj!=NULL) // incase previous thread is running, exit it before transitioning
    {
        StopAcquisition();
        while(this->scannerObj!=NULL)//wait to for the exit of prev thread
            QCoreApplication::processEvents();
    ...
    if (!pstCard->bSetError)
    {
        ...
        */
        if (success)
        {
            ...
            */
            ...
            if(daqInfoPtr->freqMode == 0)
                dwError = spcm_dwSetParam_i32 (pstCard->hDrv, SPC_TRIG_DELAY, 208);//208
            else if(daqInfoPtr->freqMode == 1)
            {
                if (daqInfoPtr->Range == 20)
                    dwError = spcm_dwSetParam_i32 (pstCard->hDrv, SPC_TRIG_DELAY, 576);//576
                else if (daqInfoPtr->Range == 50)
                    dwError = spcm_dwSetParam_i32 (pstCard->hDrv, SPC_TRIG_DELAY, 504);//576
            }
            else if(daqInfoPtr->freqMode == 2)
                dwError = spcm_dwSetParam_i32 (pstCard->hDrv, SPC_TRIG_DELAY, 32);
        ...
        */
        if (success && !dwError)
        {
            if(OsciModeEnArg)
            {
                success = bSpcMSetupModeRecFIFOMulti(pstCard, 1,(int64)SAMPLESPERPOINT , (int64)(SAMPLESPERPOINT-8),0);
            ...
            {
                success = bSpcMSetupModeRecFIFOMulti(pstCard, 1,(int64)SAMPLESPERPOINT , (int64)(SAMPLESPERPOINT-8),daqInfoPtr->ScanPoints);
                dwError=spcm_dwSetParam_i32 (pstCard->hDrv, SPC_PRETRIGGER, 8);
        ...
        */
        ...
        if (success && !dwError)
        {
            success = bSpcMSetupPathInputCh (pstCard, 0, 0, 1000, true, true, false,false); // setup for M3i card series including new features
            //success = bSpcMSetupPathInputCh (pstCard, 1, 0, 1000, false, true, false,false); // setup for M3i card series including new features
        }
...
{
  scannerObj = new fetcher;
  fetcherThread = new QThread;
  scannerObj->moveToThread(fetcherThread);

  connect(scannerObj, SIGNAL(errorSignal(QString)), this, SLOT(errorHandler(QString)));
  connect(scannerObj, SIGNAL(newArrival(int,long)), this, SLOT(newArrivalInDaq(int,long)));
  connect(scannerObj, SIGNAL(finished(int)), this, SLOT(acqFinished(int)));
  connect(scannerObj, SIGNAL(finished(int)), fetcherThread, SLOT(quit()));
  connect(scannerObj, SIGNAL(finished(int)), scannerObj, SLOT(deleteLater()));
  connect(fetcherThread, SIGNAL(finished()), fetcherThread, SLOT(deleteLater()));

  if (this->OsciModeEn==false)
     connect(fetcherThread, SIGNAL(started()), scannerObj, SLOT(fetch()));
  else
     connect(fetcherThread, SIGNAL(started()), scannerObj, SLOT(fetchOsciMode()));


  scannerObj->setPars(this->pstCard,daqInfoPtr->ScanPoints,binaryWfmBuff,OsciModeEn,dwDataBufLen);
...
void daqController::wfmCopyDone(int waveformNumber)
{
    ...
    //if (!isDemoCard)
    dwError = spcm_dwSetParam_i32(pstCard->hDrv, SPC_DATA_AVAIL_CARD_LEN, this->dwDataNotify);
    if (this->scannerObj != NULL)
        scannerObj->resetWaitForCopy();


#ifdef DAQ_DEBUG_LOGS
    if (!this->OsciModeEn)
        qDebug()<<QDateTime::currentDateTime().time()<<"daqController::wfmCopyDone waveformNumber:"<<waveformNumber<<"dwError"<<dwError;
...
void fetcher::fetch()
{
    ...
    */
    qDebug()<<"fetcher::fetch ready and waiting";
