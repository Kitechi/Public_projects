...
#if ACTUALSYSTEM
    emit setStagePosRot(x/scanInfoPtr->scanInterval*((float)360/(float)scanInfoPtr->xLengthPreCalc));
...
void dataProcessor::newWfmCopyToArray_slot(short *wfmPtr, int waveformNumber)
{
    ...
    for (unsigned int framePoint = 0 ;framePoint<blockSize;framePoint++)
    {
        ...
        isOddLine =(destFramePoint/scanInfoPtr->xLengthPreCalc) % 2;
        //if (isOddLine)
        ...
        if (((destFramePoint+11)%(scanInfoPtr->xLengthPreCalc)) == 0)
            updateResult = true;
        else if (((destFramePoint+1)%(scanInfoPtr->xLengthPreCalc)) == 0)
            updateResult = false;
        ...
        if(  (((destFramePointIncrOnly+1)%(scanInfoPtr->xLengthPreCalc)) == 0) )
        {
