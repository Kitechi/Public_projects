...
public:
    ...
    virtual QwtText trackerTextF( const QPointF &pos ) const
    {
        ...
        if ( (isIntData == 0 &&shortframePointer!=NULL ) || (isIntData == 1 &&intframePointer!=NULL ))
        {
            ...
            {
                if (isIntData)
                    text = QString::number(intframePointer[((int)(pos.y())+(int)(pos.x()*scanInfoPtr->yLengthPreCalc))]);
                    else
                    text = QString::number(shortframePointer[((int)(pos.y())+(int)(pos.x()*scanInfoPtr->yLengthPreCalc))]);
            }
        }
        */
...
public:
    ...
    //void updateDataAxis();
    mydata(int intensity = 0, int yLengthPreCalc = 50, int xLengthPreCalc = 50)
    {
        this->intensity     = intensity;
        this->xLengthPreCalc      = xLengthPreCalc;
        this->yLengthPreCalc      = yLengthPreCalc;
    ...
    void updateDataAxis()
    {
        setInterval( Qt::XAxis, QwtInterval( 0, (xLengthPreCalc-0.01) ) );
        setInterval( Qt::YAxis, QwtInterval( 0, (yLengthPreCalc-0.01) ) );
        if (isIntData)
...
*/
        if ( (isIntData == 0 && shortframePointer!=NULL ) || (isIntData == 1 &&intframePointer!=NULL ))
        {
            ...
            if (isIntData)
                return((double)(intframePointer[((y_pos)+(x_pos*yLengthPreCalc))])); // change a straight frame to 2D frame
                else
                return((double)(shortframePointer[((y_pos)+(x_pos*yLengthPreCalc))])); // change a straight frame to 2D frame

/*
            if (isIntData)
                return((double)(intframePointer[((x_pos)+(y_pos*xLengthPreCalc))])); // change a straight frame to 2D frame
                else
                return((double)(shortframePointer[((x_pos)+(y_pos*xLengthPreCalc))])); // change a straight frame to 2D frame
*/
        }
        else
 ...
 * //Median Filter Coding
    ...
    inline short imageFilterOpt(char FiltType, char krnlRadius, int x_pos, int y_pos) const
    {
        ...
        if ( (x_pos < krnlRadius) ||
             (y_pos < krnlRadius) ||
             (y_pos >= (scanInfoPtr->yLengthPreCalc-krnlRadius)) ||
             (x_pos >= (scanInfoPtr->xLengthPreCalc-krnlRadius)))
        {
            return framePointer[((x_pos)+(y_pos*scanInfoPtr->xLengthPreCalc))]; // no filtering at the borders
        }
        ...
        for (yKernel = top; yKernel<=bottom; yKernel++)
        {
            ...
            {
                kernelData[vectorIndex++] = framePointer[xKernel+yKernel*scanInfoPtr->xLengthPreCalc];
            }
        }
    ...
    inline short imageFilter(char FiltType, char krnlSz, int x_pos, int y_pos) const
    {
        ...
        if ( (x_pos < radius) ||
             (y_pos < radius) ||
             (y_pos >= (scanInfoPtr->yLengthPreCalc-radius)) ||
             (x_pos >= (scanInfoPtr->xLengthPreCalc-radius)))
        {
            return framePointer[((x_pos)+(y_pos*scanInfoPtr->xLengthPreCalc))]; // no filtering at the borders
        }

        top     = max(y_pos-radius,0);
        bottom  = min(y_pos+radius,scanInfoPtr->yLengthPreCalc-1);

        left    = max(x_pos-radius,0);
        right   = min(x_pos+radius,scanInfoPtr->xLengthPreCalc);
        ...
        for (yKernel = top; yKernel<=bottom; yKernel++)
        {
            for (xKernel = left; xKernel<=right; xKernel++)
            {
                kernelVector.insert(vectorIndex++,framePointer[xKernel+yKernel*scanInfoPtr->xLengthPreCalc]);
            }
        }
...
public:
    ...
    MyScaleDraw()
    {
        ...
        //setTickLength( QwtScaleDiv::MajorTick, 30 );
        //setTickLength( QwtScaleDiv::MinorTick, 5 );
