    ...
    QwtPlot( parent )
{
    ...
    data = new mydata(500,scanInfoPtr->yLengthPreCalc,scanInfoPtr->xLengthPreCalc);
...
*/
    ...
    zoomer->zoom(QRectF::QRectF(0,0,scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc));
    zoomer->setZoomBase(QRectF::QRectF(0,0,scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc));

    qDebug()<<"spectrogram::spectrogram - scanInfoPtr->xLengthPreCalc"<<scanInfoPtr->xLengthPreCalc<<"scanInfoPtr->yLengthPreCalc"<<scanInfoPtr->yLengthPreCalc
...
void spectrogram::selectPoint( QPointF Pos )
{
   ...
   // Pos = Pos*info_Inspection.scanInterval.toFloat();
    ...
    //SelectedImpingePoint = (scanInfoPtr->xLengthPreCalc) * ((scanInfoPtr->yLengthPreCalc - 1)  - PosYInt)  + (PosXInt);
    //SelectedImpingePoint = (ScanWidth-1) * ((ScanHeight-2*(info_Inspection.scanInterval.toFloat())) - PosYInt)  + (PosXInt+1);
    //SelectedImpingePoint = (scanInfoPtr->xLengthPreCalc) * (PosYInt)  + (PosXInt);
    //SelectedImpingePoint = (PosXInt)  + (scanInfoPtr->xLengthPreCalc)*(PosYInt);
    SelectedImpingePoint = (PosYInt)  + (scanInfoPtr->yLengthPreCalc)*(PosXInt);


    if ( PosXInt >= 0 && PosYInt >= 0 && PosXInt < scanInfoPtr->xLengthPreCalc && PosYInt < scanInfoPtr->yLengthPreCalc)
    {
...
void spectrogram::updateAxisXY(int enlargeFactor,bool calcSize, bool makePotrait)
{
    ...
    data->xLengthPreCalc = scanInfoPtr->xLengthPreCalc;
    data->yLengthPreCalc = scanInfoPtr->yLengthPreCalc;
    data->updateDataAxis();
    ...
    zoomer->yLengthPreCalc      =   scanInfoPtr->yLengthPreCalc; // just for debug
    ...
    float HtoWratio = (float)scanInfoPtr->yLengthPreCalc/(float)scanInfoPtr->xLengthPreCalc; //YtoXRatio
    float WtoHratio = (float)scanInfoPtr->xLengthPreCalc/(float)scanInfoPtr->yLengthPreCalc;
...
*/
        ...
        int inchWidth = (scanInfoPtr->xLengthPreCalc/25.4)*(logicalDpiX()*1.41);
        int inchHeight = (scanInfoPtr->yLengthPreCalc/25.4)*(logicalDpiY()*1.41);
    ...
    {
        if (scanInfoPtr->xLengthPreCalc == scanInfoPtr->yLengthPreCalc)
        {
        ...
        {
            if (scanInfoPtr->yLengthPreCalc<scanInfoPtr->xLengthPreCalc && HtoWratio < 0.5)
                makePotraitMem = makePotrait = true;
            else
                makePotraitMem = makePotrait = false;

            if (makePotrait)//(scanInfoPtr->yLengthPreCalc<scanInfoPtr->xLengthPreCalc) // one on top of other
            {
            ...
            else //if (scanInfoPtr->xLengthPreCalc<scanInfoPtr->yLengthPreCalc)
            {
                newHeight = canHeight;
                newWidth = (newHeight-2*(HEIGHTOFBOTTOMXAXIS))*(WtoHratio) + (WIDTHOFYAXISANDCOLOURBARNOTITLE);
    ...
    qDebug()<<this->objectName()<<"noOfSlavesArg"<<noOfSlaves<<"spectrogram::updateAxisXY - scanInfoPtr->xLengthPreCalc"<<scanInfoPtr->xLengthPreCalc<<"scanInfoPtr->yLengthPreCalc"<<scanInfoPtr->yLengthPreCalc
           <<"leftPos"<<leftPos
           <<"topPos"<<topPos
           <<"canvasWidth"<<canvas()->width()
           <<"canvasHeight"<<canvas()->height()
...
/*
scanInfoPtr->xLengthPreCalc 101 scanInfoPtr->yLengthPreCalc 101 canvasWidth 400 396 canvasHeight 360 356 //colourBar
scanInfoPtr->xLengthPreCalc 101 scanInfoPtr->yLengthPreCalc 101 canvasWidth 426 canvasHeight 360 // no titleforColourBar
scanInfoPtr->xLengthPreCalc 101 scanInfoPtr->yLengthPreCalc 101 canvasWidth 510 506 canvasHeight 360 356 //noColourBar
*/
