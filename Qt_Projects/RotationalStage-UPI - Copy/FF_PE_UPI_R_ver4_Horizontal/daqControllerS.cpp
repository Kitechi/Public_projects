...
{
    qDebug()<<"daqController::Configure";
        ...
        */
        if (success)
        {
            ...
            {   //SET TRIGGER CH-3
                ...
                //--jh--success = bSpcMSetupTrigChannel(pstCard,1,SPC_TM_POS);
        ...
        */
        ...
        else
        {
           //read trriger counter
            ...
            qDebug("daqController::Configure -- Sampling rate set to %.1lf MHz, binaryWfmBuff: 0x%x, OscilloscopeMode: %d, daqTrigDelay: %d, daqVoltage: %d, scanPoints: %d, dwDataNotify: %d bytes",
                   (double) pstCard->llSetSamplerate / 1000000,binaryWfmBuff,this->OsciModeEn,daqInfoPtr->daqTrigDelay,daqInfoPtr->daqVoltage,daqInfoPtr->ScanPoints,dwDataNotify);
...
void daqController::configFetchThread()
{
  ...
  connect(fetcherThread, SIGNAL(finished()), fetcherObj, SLOT(deleteLater()));
...
{
    qDebug()<<"daqController::StopAcquisition()";
    if (fetcherObj!=NULL)
        fetcherObj->reset();
}
...
void daqController::newArrivalInDaq(int waveformNumber, long lAvailPos)
{
    ...
    if (OsciModeEn == false)
    {
        allowOsciUpdate = true;//always update oscilloscope
...
void fetcher::fetch()
{
    ...
    spcm_dwSetParam_i32(pstCard_f->hDrv, SPC_TIMEOUT, 30000);
...
void fetcher::fetchOsciMode()
{
    ...
    dwError = spcm_dwSetParam_i32(pstCard_f->hDrv, SPC_TIMEOUT, 10000);
    ...
    qDebug()<<"fetcher::fetchOsciMode() emit finished(0)";
    emit finished(0);
}
