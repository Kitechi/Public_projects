    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(rotStage,SIGNAL(revDone(int)),stage,SLOT(moveHorizontalStage(int)));
...
void MainWindow::setSettingStruct()
{
    ...
    settingArr[5].trigDelay             = 80;//192;  //280(50-250) 104(500-750)
    settingArr[5].daqVoltage            = 500;  //200(250-500) 500(500-750)
...
void MainWindow::calcPossibleTrigPerRev()
{
    ...
    ui->enumScanTrigPerRev->addItem(QString::number(2400));
...
//    rotInterval = chosenRotInterval[ui->enumScanTrigPerRev->currentIndex()];
    if(ui->enumScanTrigPerRev->currentText().toInt() > 0)
        rotInterval = (double)360/ui->enumScanTrigPerRev->currentText().toDouble();
...
{
    scanInfo.scanHeight         = 2*M_PI*ui->lineEditScanRadius->text().toInt();
    scanInfo.scanWidth          = ui->lineEditScanWidth->text().toFloat();
    scanInfo.scanInterval       = linInterval;
    scanInfo.yLengthPreCalc     = ui->enumScanTrigPerRev->currentText().toInt();
    scanInfo.xLengthPreCalc     = (scanInfo.scanWidth/linInterval)+1;
    //scanInfo.scanWidth        = ui->lineEditScanWidth->text().toFloat();
...
void MainWindow::on_pushButtonDaqSet_clicked()
{
    ...
    if(dataProc->allocateMem()) // MEDIANDEBUG
    {
        ...
        rotStage->startScan( rotInterval, ui->enumScanScanSpeedDegPerSec->currentText().toInt(), 1 , TRUE );
                                                                                                                                                                                                                               //QThread::msleep(2000);
