...
stageController::stageController(structScan * scanInfoPtr_arg)
{
    ...
    writePlcMem32(226,20000); //Z-axis Jog
    writePlcMem32(228,10000); //X-axis Jog
}
...
{
    writePlcMem32(226,10000); //Z-axis Jog
    setErrorReset();
...
*/
    ...
    //acc time:K150 mini stage
       if (speed == 100 && scanInfoPtr->enableTT == true)      offset = 2870; //2550
       if (speed == 250 && scanInfoPtr->enableTT == false)      offset = 7200; //7325 big //tt configuration: 250 mm/s -> offset: 6250   //7475
       if (speed == 150 && scanInfoPtr->enableTT == false)      offset = 800;
...
void stageController::moveVerticalStage(int revCounter)
{
    unsigned int newPos;
    writePlcMem32(226,20000); //Z-axis Jog
    if(revCounter == 1)
    ...
    newPos = scanStartPosZ+(revCounter*scanInfoPtr->scanInterval)*1000;
    setPosZ(newPos);

    qDebug("stageController::moveVerticalStage: revCounter:%d, scanStartPosZ: %d, newPos: %d, scanInfoPtr->scanInterval: %f  ",revCounter,scanStartPosZ,newPos,scanInfoPtr->scanInterval);
}

void stageController::moveHorizontalStage(int revCounter)
{
    unsigned int newPos;
    writePlcMem32(226,20000); //Z-axis Jog
    if(revCounter == 1)
    ...
    newPos = scanStartPosX+(revCounter*scanInfoPtr->scanInterval)*1000;
    setPosX(newPos);

    qDebug("stageController::moveHorizontalStage: revCounter:%d, scanStartPosX: %d, newPos: %d, scanInfoPtr->scanInterval: %f  ",
           revCounter,scanStartPosX,newPos,scanInfoPtr->scanInterval);
}

void stageController::setScanStartPos(unsigned int x, unsigned int z)
{
    scanStartPosX = x;
    scanStartPosZ = z;
}
