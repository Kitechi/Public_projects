...
{
    //float x = (scanInfoPtr->scanInterval*(pointToPlot%scanInfoPtr->xLengthPreCalc) );
    //float y = (scanInfoPtr->scanInterval*(pointToPlot/scanInfoPtr->xLengthPreCalc));

    float y = (scanInfoPtr->scanInterval*(pointToPlot%scanInfoPtr->yLengthPreCalc) );
    float x = (scanInfoPtr->scanInterval*(pointToPlot/scanInfoPtr->yLengthPreCalc));
...
#if ACTUALSYSTEM
    emit setStagePosRot(y/scanInfoPtr->scanInterval*((float)360/(float)scanInfoPtr->yLengthPreCalc));
    //emit setStagePosX(x*1000,true); // set stage pos x relative to scan start coordinates
    emit setStagePosX(x*1000,true); // set stage pos z relative to scan start coordinates
#endif
...
void dataProcessor::newWfmCopyToArray_slot(short *wfmPtr, int waveformNumber)
{
    ...
    int chMap[4] = {0,1,2,3};//ch-2 is at index 1 and ch-1 is at index 2
    ...
    if (waveformNumber==0) //memory sanity check once at start
    {
        ...
        if (applyRealTimeFilter)
        {
            if (subbandDecomp)
            {
                startParallelProcThread(1,1,dataBundle[3],dataBundle[0],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc); // start the real time median mask for subband 1
                startParallelProcThread(1,1,dataBundle[4],dataBundle[1],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc); // start the real time median mask for subband 2
                startParallelProcThread(1,1,dataBundle[5],dataBundle[2],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc); // start the real time median mask for subband 3
            }
            else
            {
                startParallelProcThread(1,1,dataBundle[1],dataBundle[0],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc);
            }
        }
    }
    ...
    for (unsigned int framePoint = 0 ;framePoint<blockSize;framePoint++)
    {
        ...
        isOddLine =(destFramePoint/scanInfoPtr->yLengthPreCalc) % 2;
        //if (isOddLine)
        ...
        //else //for even rows
            //wfmToFrameConvFactor_oddLine = scanInfoPtr->yLengthPreCalc-1;


        dispBuffIndex = 0; //always show buffer 1 while acquiring the signal

        if (((destFramePoint+11)%(scanInfoPtr->yLengthPreCalc)) == 0)
            updateResult = true;
        else if (((destFramePoint+1)%(scanInfoPtr->yLengthPreCalc)) == 0)
            updateResult = false;
        ...
        if(  (((destFramePointIncrOnly+1)%(scanInfoPtr->yLengthPreCalc)) == 0) )
        {
...
void dataProcessor::rptScanFinished()
{
    ...
    this->bufToProcess = 0;
    startProcThread(1,1,dataBundle[0],dataBundle[0],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc); // start the post processing on band1
    */
}
...
void dataProcessor::scanFinished(int waveformNumber)
{
   ...
   // }
    ...
    if (this->subbandDecomp==true)
    {
         ...
         this->defaultFilter= true;
         this->bufToProcess = 1;
         //startProcThread(1,1,dataBundle[1],dataBundle[1],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc); // start the post processing on band1
    }
    else
    {
         chosenSubband = 0;
        ...
        resultInfoPtr->filterItr2        != this->filterItr2))
    {
        ...
        if (daqInfoPtr->subbandDecomp)
        {
            startProcThread(resultInfoPtr->filterType,resultInfoPtr->filterRadius,
                        dataBundle[chosenSubband],dataBundle[(chosenSubband+NUMOFBANDS)],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc);
        }
        else
        {
            startProcThread(resultInfoPtr->filterType,resultInfoPtr->filterRadius,dataBundle[0],dataBundle[1],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc);
        }
    }
...
void dataProcessor::postProcessingXCorRequested()
{
    if (isMemValid(dataBundle, dispBuffIndex, frameNum,__FUNCTION__) == false)
     ...
     //int ->yLengthPreCalc = (buffScanHeight/buffScanInterval)+1;
     float y = (buffScanInterval*(this->selectedPoint%scanInfoPtr->yLengthPreCalc) );
     float x = (buffScanInterval*(this->selectedPoint/scanInfoPtr->yLengthPreCalc));
...
void dataProcessor::startParallelProcThread(short filtType ,short krnlRadius, short **Input,short **Output,int xLengthPreCalc,int yLengthPreCalc)
{
  this->coreProcObj = new coreProcessor(filtType,krnlRadius, Input,Output,scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc);
...
void dataProcessor::startProcThread(short filtType ,short krnlRadius, short **Input,short **Output,int xLengthPreCalc,int yLengthPreCalc)
{
    ...
    for (threadNum = 0;threadNum<this->totalThreads;threadNum++)
    {
        ...
        this->coreProcObjArr[threadNum] = new coreProcessor(filtType,krnlRadius, Input,Output,scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc,startFrame,endFrame) ;
    ...
    else
    {
        ...
        if (this->defaultFilter)
        {
            ...
            {//one more iteration on the same buffer
                ...
                startProcThread(1,1,dataBundle[bufToProcess],dataBundle[bufToProcess],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc);
        ...
        else
        {
            ...
            {
                startProcThread(resultInfoPtr->filterType,resultInfoPtr->filterRadius,
                               dataBundle[FilterBuffNum],dataBundle[FilterBuffNum],scanInfoPtr->xLengthPreCalc,scanInfoPtr->yLengthPreCalc);
...
//                    Qt::WindowFlags flags = Qt::Dialog | Qt::MSWindowsFixedSizeDialogHint);
    ...
    if (fd->exec())
    {
        ...
        //save the data related settings necessary for loading the data next time.
         ...
         scanInfoPtr->yLengthPreCalc = ceil(buffScanHeightLoc/buffScanIntervalLoc);
         scanInfoPtr->xLengthPreCalc = ceil(daqInfoPtr->ScanPoints/scanInfoPtr->yLengthPreCalc);
         ...
         qDebug()<<"dataProcessor::loadData"<<
                   ...
                   "scanInfoPtr->yLengthPreCalc"<<scanInfoPtr->yLengthPreCalc<<
                   "scanInfoPtr->xLengthPreCalc"<<scanInfoPtr->xLengthPreCalc;
...
coreProcessor::coreProcessor(char filtType, char krnlRadius, short **input, short **output,
                             int xLengthPreCalc_arg, int yLengthPreCalc_arg,int startFrame, int endFrame,unsigned int buffScanPoints,unsigned int * EnhancedAlgoResultArg,
                             structResult *resultInfoPtr_arg,int selectedPt_arg)
{
    ...
    this->input       =  input;
    this->output      =  output;
    this->xLengthPreCalc    =  xLengthPreCalc_arg;
    this->yLengthPreCalc    =  yLengthPreCalc_arg;
...
{
    int x_length = xLengthPreCalc;
    int y_length = yLengthPreCalc;
